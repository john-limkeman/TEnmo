...
public class TenmoController {
	...
	//returns User based on give username
	...
	//creates a Transfer type request that is pending approval
	...
	public Transfer requestTransfer(@RequestBody Transfer transfer) {
		return transfer = transferDAO.requestTransfer(transfer.getAccountFrom(), transfer.getAccountTo(), transfer.getAmount());
	}
	
	//creates a Transfer type send that is pending approval
	@ResponseStatus(HttpStatus.CREATED)
	...
	// returns a list of transfers status pending interacting with a given user id
